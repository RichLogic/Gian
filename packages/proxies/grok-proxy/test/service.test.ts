...
import { GrokProxyService, parseGrokConversationUsage } from '../src/core/service.js';
...
test('versioned ACP prompt usage parses known fields and rejects unknown shapes', () => {
  assert.deepEqual(parseGrokConversationUsage(ACP_V0_23_PROMPT_USAGE.usage), {
    mode: 'absolute',
  ...
  assert.equal(parseGrokConversationUsage(ACP_UNKNOWN_PROMPT_USAGE.usage), null);
  assert.equal(parseGrokConversationUsage(ACP_MALFORMED_V0_23_PROMPT_USAGE.usage), null);
  assert.equal(parseGrokConversationUsage({ inputTokens: '1100000' }), null);
  assert.equal(parseGrokConversationUsage({
    inputTokens: 0.5,
    outputTokens: 0,
    totalTokens: 0,
  }), null);
...
function transportFactory(agentFactory: (client: AgentSideConnection) => Agent) {
  const factory: GrokAcpTransportFactory = async (client: Client) => {
...
test('captures pre-response command updates and load replay without emitting provisional events', async () => {
  ...
  const runtime = new GrokAcpClient({
...
test('a failed native load leaves no attached proxy row', async () => {
  ...
  const service = new GrokProxyService({ runtime });
...
test('maps ACP auth_required to the stable proxy AUTH_REQUIRED code', async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => (
      ...
      && 'message' in error
      && String(error.message).includes("'/managed/grok' login")
    ),
  );
    ...
    transportFactory: transportFactory((remote) => ({
      ...
      prompt: async (params: { sessionId: string }) => {
        const response = remote.requestPermission({
          ...
          options: [
            { optionId: 'grok-once-42', name: 'Allow once', kind: 'allow_once' },
            { optionId: 'grok-no-42', name: 'Reject', kind: 'reject_once' },
          ],
        });
...
const FULL_CONFIG_OPTIONS = [
  {
    ...
    currentValue: 'grok-k2',
    options: [
      { value: 'grok-k2', name: 'Grok K2' },
      { value: 'grok-k2-thinking', name: 'Grok K2 Thinking' },
    ],
  },
  {
    type: 'select',
...
test('capabilities probes model and thinking choices from configOptions', async () => {
  ...
  assert.deepEqual(capabilities.models[1], {
    ...
    id: 'grok-model-grok-k2-thinking',
    model: 'grok-k2-thinking',
    displayName: 'Grok K2 Thinking',
    isDefault: false,
  });
  await service.close();
});
...
test('Grok gian.proxy/1 translates ACP text, tools, usage, and Host ids', async () => {
  let remote!: AgentSideConnection;
  const runtime = new GrokAcpClient({
    binaryPath: '/managed/grok',
    transportFactory: transportFactory((client) => {
  ...
  const adapter = new GrokProtocolV1Adapter(service, '0.1.0', notification => {
  ...
  await waitFor(
    ...
    'standard Grok turn did not complete',
  );
  ...
  for (const notification of notifications) {
    if ('turnId' in notification.params) {
      assert.equal(notification.params.turnId, 'host-grok-turn');
    }
  }
  await service.close();
});

test('Grok gian.proxy/1 detaches a newly-created service session when config fails', async () => {
  const runtime = new GrokAcpClient({
    binaryPath: '/managed/grok',
    transportFactory: transportFactory(() => ({
      initialize: async () => initializeResponse(),
  ...
  const adapter = new GrokProtocolV1Adapter(service, '0.1.0', () => undefined);
  await adapter.handle(v1Request(1, 'initialize', {
    protocol: { name: PROTOCOL_NAME, versions: [PROTOCOL_V1] },
    host: { name: 'Gian', version: '9.9.9' },
  }));
...
test('Grok gian.proxy/1 returns normalized replay on one synthetic stream', async () => {
  let remote!: AgentSideConnection;
  const runtime = new GrokAcpClient({
    binaryPath: '/managed/grok',
    transportFactory: transportFactory((client) => {
...
test('Grok gian.proxy/1 rejects a yolo mode that would relax Host relay/ask policy', async () => {
  const runtime = new GrokAcpClient({
    binaryPath: '/managed/grok',
    transportFactory: transportFactory(() => ({
      initialize: async () => initializeResponse(),
      newSession: async () => ({ sessionId: 'native-policy' }),
      prompt: async () => {
        throw new Error('turn must not start after a policy rejection');
      },
    } as unknown as Agent)),
  });
  const service = new GrokProxyService({ runtime });
  ...
  await assert.rejects(
    adapter.handle(v1Request(3, 'turn.start', {
      sessionId: 'host-policy',
      streamId: created.session.streamId,
      turnId: 'host-policy-turn',
      input: [{ type: 'text', text: 'go' }],
      policy: { workspaceRoots: ['/tmp'], approval: 'relay', network: 'ask' },
      config: { mode: 'yolo', native: {} },
    })),
    (error: unknown) => (
      error instanceof ProxyProtocolError
      && error.code === 'POLICY_NOT_SUPPORTED'
    ),
  );
  await assert.rejects(
    adapter.handle(v1Request(4, 'turn.start', {
      sessionId: 'host-policy',
      streamId: created.session.streamId,
      turnId: 'host-policy-native',
      input: [{ type: 'text', text: 'go' }],
      policy: { workspaceRoots: ['/tmp'], approval: 'relay', network: 'ask' },
      config: { native: { mode: 'yolo' } },
    })),
    (error: unknown) => (
      error instanceof ProxyProtocolError
      && error.code === 'POLICY_NOT_SUPPORTED'
  ...
  await assert.rejects(
    adapter.handle(v1Request(5, 'turn.start', {
      sessionId: 'host-policy',
      streamId: created.session.streamId,
      turnId: 'host-policy-root',
      input: [{ type: 'text', text: 'go' }],
      policy: { workspaceRoots: ['/tmp', '/outside'], approval: 'relay', network: 'ask' },
      config: { native: {} },
    })),
    (error: unknown) => (
      error instanceof ProxyProtocolError
...
test('Grok gian.proxy/1 rejects a session root the workspace sandbox cannot enforce', async () => {
  const runtime = new GrokAcpClient({
    binaryPath: '/managed/grok',
    transportFactory: transportFactory(() => ({
      initialize: async () => initializeResponse(),
      newSession: async () => {
        throw new Error('session must not start after a workspace policy rejection');
      },
    } as unknown as Agent)),
  });
  const service = new GrokProxyService({ runtime });
...
test('Grok gian.proxy/1 maps mid-turn ACP auth to RUNTIME_AUTH_REQUIRED', async () => {
  const runtime = new GrokAcpClient({
    binaryPath: '/managed/grok',
    transportFactory: transportFactory(() => ({
      initialize: async () => initializeResponse(),
      newSession: async () => ({ sessionId: 'native-turn-auth' }),
      prompt: async () => {
        throw RequestError.authRequired();
      },
    } as unknown as Agent)),
  ...
  await waitFor(
    ...
    'auth turn failure was not emitted',
  );
  const failed = notifications.find(item => item.method === 'turn.failed');
  assert.equal(failed?.method, 'turn.failed');
  if (failed?.method !== 'turn.failed') throw new Error('expected turn.failed');
  assert.equal(failed.params.data.error.code, 'RUNTIME_AUTH_REQUIRED');
  assert.match(failed.params.data.error.message, /\/managed\/grok' login/);
  await service.close();
});
