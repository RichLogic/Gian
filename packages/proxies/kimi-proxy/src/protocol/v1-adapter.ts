...
export class KimiProtocolV1Adapter {
  ...
  private async createSession(
    ...
    for (const [id, value] of Object.entries(params.config)) {
      if (typeof value !== 'string' && typeof value !== 'boolean') {
        throw new ProxyProtocolError(
          'INVALID_REQUEST',
          `Kimi config ${id} must be string or boolean.`,
          false,
        );
      }
    ...
    try {
      ...
      try {
        ...
        throw new AggregateError(
          [error, cleanupError],
          'Kimi session configuration failed and cleanup did not complete.',
        );
      }
      throw error;
    }
  ...
  private async setConfig(
    ...
    if (typeof params.value !== 'string' && typeof params.value !== 'boolean') {
      throw new ProxyProtocolError('INVALID_REQUEST', 'Kimi config values must be string or boolean.', false);
    }
    ...
    if (method === 'runtime.stopped') {
      ...
      this.emitEvent(proxyNotificationSchema.parse({
        ...
        params: {
          ...
          data: {
            code: 'RUNTIME_ERROR',
            message: String(data.error ?? 'Kimi ACP process stopped unexpectedly.'),
            retryable: true,
            data: jsonValue(data),
          },
        },
  ...
  private translateTool(
    ...
    this.emitSessionEvent('tool.completed', session, turnId, {
      ...
      ...(status === 'failed' ? {
        error: {
          code: 'RUNTIME_ERROR',
          message: String(update.title ?? 'Kimi tool failed.'),
          retryable: false,
          data: {},
        },
      } : {}),
...
export function kimiProtocolError(error: unknown) {
