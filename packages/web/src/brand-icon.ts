...
const DEV_BADGE_SVG = [
  '<g data-gian-dev-badge="true">',
  '<rect x="824" y="54" width="364" height="214" rx="107" fill="#101111" stroke="#fff" stroke-width="20"/>',
  '<text x="1006" y="196" text-anchor="middle" fill="#fff" font-family="-apple-system,BlinkMacSystemFont,Arial,sans-serif" font-size="104" font-weight="800" letter-spacing="5">DEV</text>',
  '</g>',
].join('');
...
export function buildGianIconSvg(
  ...
  return [
    ...
    `<path d="${GIAN_DRAGON_BODY_PATH}" fill="#101111" fill-rule="evenodd"/>`,
    `<path d="${GIAN_DRAGON_WHISKER_TOP_PATH}" fill="#101111"/>`,
    `<path d="${GIAN_DRAGON_WHISKER_BOTTOM_PATH}" fill="#101111"/>`,
    development ? DEV_BADGE_SVG : '',
    '</svg>',
  ].join('');
}
...
function renderDockIcon(
  ...
  context.fill(new Path2D(GIAN_DRAGON_WHISKER_BOTTOM_PATH));
  context.restore();

  if (development) {
    const badgeWidth = tileSize * 0.36;
    const badgeHeight = tileSize * 0.20;
    const badgeX = inset + tileSize - badgeWidth * 0.94;
    const badgeY = inset - badgeHeight * 0.08;
    const badgeRadius = badgeHeight / 2;
    context.beginPath();
    context.roundRect(badgeX, badgeY, badgeWidth, badgeHeight, badgeRadius);
    context.fillStyle = '#101111';
    context.fill();
    context.lineWidth = Math.max(4, tileSize * 0.014);
    context.strokeStyle = '#fff';
    context.stroke();
    context.fillStyle = '#fff';
    context.font = `800 ${badgeHeight * 0.48}px -apple-system, BlinkMacSystemFont, Arial, sans-serif`;
    context.textAlign = 'center';
    context.textBaseline = 'middle';
    context.fillText('DEV', badgeX + badgeWidth / 2, badgeY + badgeHeight * 0.53);
  }
