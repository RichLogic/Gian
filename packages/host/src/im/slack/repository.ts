...
interface SlackInboundEventRow {
  id: string;
  bot_id: string;
  kind: string;
...
export interface QueuedSlackTurnRecord {
  id: string;
  sessionId: string;
  ownerUserId: string;
  prompt: string | null;
...
export interface PersistedSlackCodingTurnRecord {
  id: string;
  sessionId: string;
  turnId: string;
  threadId: string;
...
export interface SlackCodingTurnProjection {
  turnId: string;
  threadId: string;
  status: string;
  objective: string | null;
...
function toAdminBotRecord(record: SlackBotRecord): AdminSlackBotRecord {
  return {
    id: record.id,
    ...
    allowedSlackUserId: record.allowedSlackUserId,
    selectedWorkspaceId: record.selectedWorkspaceId,
    directChannelId: record.directChannelId,
    commandPrefix: record.commandPrefix,
    enabled: record.enabled,
    hasBotToken: record.botTokenCiphertext.trim().length > 0,
    hasAppToken: record.appTokenCiphertext.trim().length > 0,
    hasConfigToken: (record.configTokenCiphertext ?? '').trim().length > 0,
...
function asSessionRecord(row: SlackSessionRow): SessionRecord {
  return {
    id: row.id,
    ownerUserId: row.owner_user_id,
    ownerUsername: row.owner_username,
...
function asTurnRecord(row: SlackTurnRow): PersistedSlackCodingTurnRecord {
  return {
    id: row.id,
...
function asQueuedTurnRecord(row: QueuedSlackTurnRow): QueuedSlackTurnRecord {
  return {
    id: row.id,
    sessionId: row.session_id,
    ownerUserId: row.owner_user_id,
...
export class SlackCodingRepository {
  ...
  async getBotRecordForOwner(ownerUserId: string) {
    const row = this.db.prepare(`
      SELECT * FROM slack_bots
      WHERE owner_user_id = ?
    `).get(ownerUserId) as SlackBotRow | undefined;
    return row ? asBotRecord(row) : null;
  }

  async createBot(record: SlackBotRecord) {
    try {
      this.db.prepare(`
      ...
      `).run(
        record.id, record.ownerUserId, record.ownerUsername, record.label,
        record.botTokenCiphertext, record.appTokenCiphertext,
        record.teamId, record.botUserId, record.allowedSlackUserId,
        record.selectedWorkspaceId, record.selectedSessionId, record.directChannelId,
        record.commandPrefix, record.configTokenCiphertext,
        toSqliteBoolean(record.enabled), record.status,
        record.lastError, record.lastConnectedAt,
        record.createdAt, record.updatedAt,
      );
    } catch (error) {
      if (isSqliteUniqueConstraintError(error)) {
        throw new Error('Slack bot record conflicts with existing data.');
      }
      throw error;
    }
  ...
  async updateBot(botId: string, patch: Partial<SlackBotRecord>) {
    ...
    this.db.prepare(`
      ...
      WHERE id = ?
    `).run(
      next.ownerUsername, next.label,
      next.botTokenCiphertext, next.appTokenCiphertext,
      next.teamId, next.botUserId, next.allowedSlackUserId,
      next.selectedWorkspaceId, next.selectedSessionId, next.directChannelId,
      next.commandPrefix, next.configTokenCiphertext,
      toSqliteBoolean(next.enabled), next.status,
      next.lastError, next.lastConnectedAt,
      next.updatedAt, botId,
    );
  ...
  async deleteBot(botId: string) {
    const result = this.db.prepare(`
      DELETE FROM slack_bots WHERE id = ?
  ...
  async countBotsForUser(userId: string) {
    const row = this.db.prepare(`
      SELECT COUNT(*) AS count FROM slack_bots WHERE owner_user_id = ?
  ...
  async countSessionsForUser(userId: string) {
    const row = this.db.prepare(`
      SELECT COUNT(*) AS count FROM slack_coding_sessions WHERE owner_user_id = ?
    `).get(userId) as { count: number };
    return row.count;
  }
  ...
  async updateOwnerUsername(userId: string, ownerUsername: string) {
    ...
    this.db.prepare(`
      UPDATE slack_bots SET owner_username = ?, updated_at = ? WHERE owner_user_id = ?
    `).run(ownerUsername, now, userId);
    this.db.prepare(`
      UPDATE slack_coding_sessions SET owner_username = ?, updated_at = ? WHERE owner_user_id = ?
    `).run(ownerUsername, now, userId);
  }
  ...
  async getSession(sessionId: string) {
    const row = this.db.prepare(`
      SELECT * FROM slack_coding_sessions WHERE id = ?
    `).get(sessionId) as SlackSessionRow | undefined;
    return row ? asSessionRecord(row) : null;
  }

  async getSessionForBot(botId: string, sessionId: string) {
    const row = this.db.prepare(`
      SELECT * FROM slack_coding_sessions WHERE bot_id = ? AND id = ?
    `).get(botId, sessionId) as SlackSessionRow | undefined;
    return row ? asSessionRecord(row) : null;
  }
  ...
  async findSessionByThreadId(threadId: string) {
    const row = this.db.prepare(`
      SELECT * FROM slack_coding_sessions
      WHERE thread_id = ?
    `).get(threadId) as SlackSessionRow | undefined;
    return row ? asSessionRecord(row) : null;
  }
  ...
  async listSessionsForUser(userId: string) {
    ...
    `).all(userId) as unknown as SlackSessionRow[];
  ...
  async listSessionsForOwnerWorkspace(userId: string, workspaceId: string) {
    ...
    `).all(userId, workspaceId) as unknown as SlackSessionRow[];
    return rows.map(asSessionRecord);
  }

  async getSessionForUser(sessionId: string, userId: string) {
    const row = this.db.prepare(`
      SELECT * FROM slack_coding_sessions WHERE id = ? AND owner_user_id = ?
    `).get(sessionId, userId) as SlackSessionRow | undefined;
    return row ? asSessionRecord(row) : null;
  }

  async upsertSession(record: SessionRecord) {
    if (!record.botId) {
      throw new Error('Slack session is missing botId.');
    }

    this.db.prepare(`
      INSERT INTO slack_coding_sessions (
        id, bot_id, owner_user_id, owner_username, executor,
        workspace_id, thread_id, active_turn_id, title, auto_title,
        workspace, archived_at, security_profile, approval_mode,
        network_enabled, full_host_enabled, status, last_issue,
        has_transcript, model, reasoning_effort, execution_mode, job_json,
        created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    `).run(
      record.id, record.botId, record.ownerUserId, record.ownerUsername, record.executor,
      record.workspaceId, record.threadId, record.activeTurnId, record.title,
      toSqliteBoolean(record.autoTitle), record.workspace, record.archivedAt,
      record.securityProfile, record.approvalMode,
      toSqliteBoolean(record.networkEnabled), toSqliteBoolean(record.fullHostEnabled),
      record.status, record.lastIssue, toSqliteBoolean(record.hasTranscript),
      record.model, record.reasoningEffort, record.executionMode ?? 'interactive',
      serializeJson(record.job ?? null), record.createdAt, record.updatedAt,
    );
  ...
  async deleteSession(sessionId: string) {
    const result = this.db.prepare(`
      DELETE FROM slack_coding_sessions WHERE id = ?
  ...
  async markAllStale(reason: string) {
    this.db.prepare(`
      UPDATE slack_coding_sessions
      SET active_turn_id = NULL, status = 'stale', last_issue = ?,
          network_enabled = 0, updated_at = ?
  ...
  async listTurns(sessionId: string) {
    const rows = this.db.prepare(`
      SELECT * FROM slack_coding_turns WHERE session_id = ? ORDER BY seq ASC
    `).all(sessionId) as unknown as SlackTurnRow[];
    return rows.map(asTurnRecord);
  }

  async mergeTurnProjections(sessionId: string, projections: SlackCodingTurnProjection[]) {
    ...
    withTransaction(this.db, () => {
      const highestSeqRow = this.db.prepare(`
        SELECT seq FROM slack_coding_turns WHERE session_id = ? ORDER BY seq DESC LIMIT 1
      `).get(sessionId) as Pick<SlackTurnRow, 'seq'> | undefined;

      let nextSeq = (highestSeqRow?.seq ?? -1) + 1;
      const now = new Date().toISOString();
      const selectExisting = this.db.prepare(`SELECT id FROM slack_coding_turns WHERE id = ?`);
      const update = this.db.prepare(`
        UPDATE slack_coding_turns
        SET thread_id = ?, status = ?, objective_text = ?, assistant_text = ?,
            thread_preview_text = ?, transcript_entries_json = ?,
            commands_json = ?, changes_json = ?, updated_at = ?
        WHERE id = ?
      `);
      const insert = this.db.prepare(`
        INSERT INTO slack_coding_turns (
          id, session_id, turn_id, thread_id, seq, status,
          objective_text, assistant_text, thread_preview_text,
          transcript_entries_json, commands_json, changes_json,
          created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);
      ...
      for (const projection of projections) {
        ...
        const payload = [
          projection.threadId, projection.status,
          projection.objective, projection.assistantText, projection.threadPreview,
          serializeJson(projection.transcriptEntries),
          serializeJson(projection.commands), serializeJson(projection.changes), now,
        ] as const;
        ...
        insert.run(
          documentId, sessionId, projection.turnId, projection.threadId, nextSeq++,
          projection.status, projection.objective, projection.assistantText,
          projection.threadPreview, serializeJson(projection.transcriptEntries),
          serializeJson(projection.commands), serializeJson(projection.changes), now, now,
        );
      }
    });
  ...
  async deleteSessionHistory(sessionId: string) {
    this.db.prepare(`DELETE FROM slack_coding_turns WHERE session_id = ?`).run(sessionId);
  }
  ...
  async listQueuedTurns(sessionId: string) {
    const rows = this.db.prepare(`
      SELECT * FROM slack_coding_queued_turns
      WHERE session_id = ? AND status = 'queued'
      ORDER BY created_at ASC, rowid ASC
    `).all(sessionId) as unknown as QueuedSlackTurnRow[];
    return rows.map(asQueuedTurnRecord);
  }

  async countQueuedTurns(sessionId: string) {
    const row = this.db.prepare(`
      SELECT COUNT(*) AS count FROM slack_coding_queued_turns
      WHERE session_id = ? AND status = 'queued'
  ...
  async enqueueTurn(input: {
    ...
    const now = new Date().toISOString();
    const record: QueuedSlackTurnRecord = {
      id: randomUUID(),
    ...
    this.db.prepare(`
      INSERT INTO slack_coding_queued_turns (
        id, session_id, owner_user_id, prompt, attachment_ids_json,
        status, queued_after_turn_id, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      record.id, record.sessionId, record.ownerUserId, record.prompt,
      serializeJson(record.attachmentIds), record.status,
      record.queuedAfterTurnId, record.createdAt, record.updatedAt,
    );
  ...
  async claimNextQueuedTurn(sessionId: string) {
    return withTransaction(this.db, () => {
      ...
      `).get(sessionId) as QueuedSlackTurnRow | undefined;
      if (!row) {
        return null;
      }
      ...
      this.db.prepare(`
        UPDATE slack_coding_queued_turns SET status = 'starting', updated_at = ? WHERE id = ?
      `).run(updatedAt, row.id);

      return asQueuedTurnRecord({ ...row, status: 'starting', updated_at: updatedAt });
    });
  }

  async deleteQueuedTurn(sessionId: string, queuedTurnId: string) {
    const result = this.db.prepare(`
      DELETE FROM slack_coding_queued_turns WHERE id = ? AND session_id = ?
  ...
  async resetQueuedTurnToQueued(sessionId: string, queuedTurnId: string) {
    const updatedAt = new Date().toISOString();
    this.db.prepare(`
      UPDATE slack_coding_queued_turns SET status = 'queued', updated_at = ?
  ...
  async deleteAllQueuedTurns(sessionId: string) {
    const result = this.db.prepare(`
      DELETE FROM slack_coding_queued_turns WHERE session_id = ? AND status = 'queued'
    `).run(sessionId) as { changes?: number };
  ...
  async getQueuedTurn(sessionId: string, queuedTurnId: string) {
    const row = this.db.prepare(`
      SELECT * FROM slack_coding_queued_turns WHERE id = ? AND session_id = ?
    `).get(queuedTurnId, sessionId) as QueuedSlackTurnRow | undefined;
  ...
  async recordInboundEvent(input: {
    ...
    try {
      this.db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `).run(input.id, input.botId, input.kind, input.channelId ?? null, input.authorId ?? null, new Date().toISOString());
      return true;
    } catch (error) {
      if (isSqliteUniqueConstraintError(error)) {
        return false;
  ...
  async createOutboxMessage(input: {
    ...
    this.db.prepare(`
      INSERT INTO slack_outbox (
        ...
        status, attempt_count, sent_message_id, last_error,
        created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      record.id, record.botId, record.sessionId, record.channelId,
      record.turnId, record.content, record.status, record.attemptCount,
      record.sentMessageId, record.lastError, record.createdAt, record.updatedAt,
    );

    return record;
  }

  async getOutboxMessageBySentMessageId(botId: string, sentMessageId: string) {
    const row = this.db.prepare(`
