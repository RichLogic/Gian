...
export interface KimiProtocolV1HostOptions {
  entry: string;
...
export class KimiProtocolV1Host {
  private readonly client: ProtocolV1Client;
  private readonly sessions = new Map<string, KimiProtocolV1SessionClient>();
  ...
  constructor(options: KimiProtocolV1HostOptions) {
  ...
  createSessionClient(hostSessionId: string): KimiProtocolV1SessionClient {
    const facade = new KimiProtocolV1SessionClient(this, hostSessionId);
  ...
  async capabilities(): Promise<KimiCapabilities> {
  ...
  unregister(sessionId: string, facade: KimiProtocolV1SessionClient): void {
...
export class KimiProtocolV1SessionClient implements ProxyClient {
  ...
  constructor(
    private readonly host: KimiProtocolV1Host,
    private readonly hostSessionId: string,
  ) {}

  runtimeHost(): KimiProtocolV1Host { return this.host; }
  ...
  capabilities(): Promise<KimiCapabilities> { return this.host.capabilities(); }
  ...
  async createSession(params: CreateSessionParams) {
    ...
    if (!nativeSessionId) {
      throw new Error('Kimi gian.proxy/1 session.create omitted nativeSession.id.');
    }
  ...
  async startTurn(params: StartTurnParams) {
    ...
    if (!turnId) throw new Error('Kimi gian.proxy/1 turn.start requires a Host turnId.');
    const cwd = this.cwd;
    if (!cwd) throw new Error('Kimi gian.proxy/1 session has no cwd.');
  ...
  async respondApproval(params: RespondApprovalParams): Promise<void> {
    if (!params.nativeOptionId) {
      throw new Error('Kimi gian.proxy/1 approvals require the exact Proxy option id.');
    }
  ...
  private async loadReplay(): Promise<{ replayStreamId: string; events: ProxyNotification[] }> {
    ...
    for (let pageIndex = 0; pageIndex < 100; pageIndex += 1) {
      ...
      if (seenCursors.has(page.nextCursor)) {
        throw new Error(`Kimi replay cursor ${page.nextCursor} was repeated.`);
      }
      seenCursors.add(page.nextCursor);
      cursor = page.nextCursor;
    }
    throw new Error('Kimi replay exceeded 100 pages.');
  }

  private requireStream(): string {
    if (!this.stream) throw new Error('Kimi gian.proxy/1 session is not attached.');
    return this.stream;
  }

  private requireTurn(): string {
    if (!this.activeTurnId) throw new Error('Kimi gian.proxy/1 session has no active turn.');
